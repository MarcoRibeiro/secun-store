      ...
      <div className="relative overflow-hidden border border-slate-200 bg-white px-6 py-14 shadow-sm small:px-12">
        ...
        <div className="relative flex flex-col justify-between gap-8 small:flex-row small:items-center">
          <div className="max-w-3xl">
            ...
            <p className="mt-4 text-base-regular text-slate-600">
              {content.home.promoText}
            </p>
            <div className="mt-6 grid gap-3 medium:grid-cols-3">
              {content.home.promoHighlights.map((item) => (
                <div
                  key={item}
                  className="border border-slate-200 bg-white/80 px-4 py-3 text-sm font-medium text-slate-700"
                >
                  {item}
                </div>
              ))}
          ...
          <LocalizedClientLink
            href="/store"
            className="inline-flex w-max shrink-0 items-center gap-2 rounded-md border border-sky-500 px-5 py-3 text-sm font-semibold text-sky-700 transition hover:bg-sky-500 hover:text-white"
          >
